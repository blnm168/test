        ...
        <column-wrapper slot="row"
          ...
          <template v-for="(dict, index) in dataSet">
            <Column v-if="(dict.data=='handle')"
                    ...
                    v-bind:key="index">
              <a href="javascript:;"
                 @click="linkPlanTransfer(scope.row)">查看详情</a>
            </Column>
            <Column v-else
                    :data="dict.data"
...
<script>
import { dataSet } from './plan-transfer.config.js'
import { planTransferTable } from './request.js'
...
export default {
  ...
  methods: {
    ...
    linkPlanTransfer () {
      ...
      const payload = {
        operationType: 'Edit',
        RequisitionPlanId: this.originId
      }
      location.assign(`${this.$scmLink}RequisitionPlanManagement/RequisitionPlanManagementAdd${this.$stringify(payload)}`)
    }
  }
}
</script>
