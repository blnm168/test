        ...
        <column-wrapper slot="row"
          ...
          <template v-for="(dict, index) in dataSet">
            <Column v-if="(dict.data=='handle')"
                    ...
                    :data="dict.data"
                    v-bind:key="index">
              <a :href='"../result/stockin-detail.html?billCode=" + scope.row.stockCode + "&token="+$token'>查看详情</a>
            </Column>
            <Column v-else
                    :data="dict.data"
...
<script>
import { dataSet } from './transfer-storage.config.js'
import { transferStorageTable } from './request.js'
...
export default {
  ...
  methods: {
    ...
    search () {
      ...
      return transferStorageTable(args)
        .then((response) => {
