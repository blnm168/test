<template>
  <app-layout>
        ...
        <Form ref="form"
          ...
          <div v-show="ifOption">
            <Row>
              ...
              <Dates title="到货日期"
                     :start.sync="condition.arrivalStartTime"
                     :end.sync="condition.arrivalEndTime">
              </Dates>
              <Dates title="完成日期"
      ...
      <app-table ref="mainTable"
        ...
        <column-wrapper slot="row"
          ...
          <template v-for="(dict, index) in dataSet">
            <!-- 转移库存 -->
            <Column v-if="(dict.data=='transferInventory')"
                    class="handle-td"
                    :data="dict.data"
                    v-bind:key="index"
                    :handleClick="triggleLockingModal"></Column>
            <Column v-else
                    :data="dict.data"
    ...
    <!-- 锁定库存弹窗 -->
    <locking-modal ref="lockingModal"></locking-modal>
  </app-layout>
</template>
...
import lockingModal from './locking.vue'

export default {
  mixins: [tableMixin],
  components: {
    'locking-modal': lockingModal
  },
  data: () => ({
    subject: '销售单',
    dataSet,
  ...
  methods: {
    ...
    triggleLockingModal (row) {
      if (!row.transferInventory) {
      ...
      this.$refs.lockingModal.init(item, id, row.originId)
    },
    toggleOption () {
